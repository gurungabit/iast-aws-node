      ...
      <div className="mx-auto max-w-4xl">
        ...
        {schedules.length === 0 ? (
          <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 py-20 text-center">
            ...
            <p className="text-sm font-medium text-gray-500 dark:text-zinc-400">No schedules found</p>
            <p className="mt-1 text-xs text-gray-400 dark:text-zinc-600">Schedules for this date will appear here</p>
          </div>
        ) : (
          <div className="rounded-xl border border-gray-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 divide-y divide-gray-100 dark:divide-zinc-800 overflow-hidden">
            {schedules.map((schedule) => (
              ...
              >
                <div>
                  <p className="text-sm font-medium text-gray-900 dark:text-zinc-100">{schedule.astName}</p>
                ...
                <span
                  className={cn(
                    ...
                    schedule.status === 'pending' && 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400',
                    schedule.status === 'completed' && 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
                    schedule.status === 'failed' && 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400',
                  )}
                >
                  {schedule.status}
                </span>
